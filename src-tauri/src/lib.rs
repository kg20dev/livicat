...
async fn open_preview_window(
    ...
    // WebView2 on Windows needs more time to initialize before CSS injection
    // Use background thread spawn to avoid blocking UI
    ...
    std::thread::spawn(move || {
        ...
        println!(
            "[Livicat] Waiting {}s before CSS injection (platform: {})",
            delay.as_secs(),
            std::env::consts::OS
        );
        ...
        match inject_css_to_window(&window_clone, &css_clone) {
            Ok(_) => println!("[Livicat] CSS injected successfully"),
            Err(e) => eprintln!("[Livicat] CSS injection failed: {}", e),
        }
    });
