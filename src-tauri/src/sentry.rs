...
pub fn capture_error(err: &str) {
    capture_message(err, SentryLevel::Error);
}
...
pub fn send_fake_crash_event() {
    ...
    add_breadcrumb(
        "crash",
        "Simulated application crash for testing",
        SentryLevel::Fatal,
    );
...
pub fn send_fake_error_with_stacktrace() {
    ...
    add_breadcrumb(
        "ui_action",
        "User clicked 'Open Preview'",
        SentryLevel::Info,
    );
    add_breadcrumb(
        "preview",
        "Attempting to create preview window",
        SentryLevel::Info,
    );
    add_breadcrumb(
        "error",
...
pub fn init_sentry() -> sentry::ClientInitGuard {
    ...
    let release = option_env!("SENTRY_RELEASE")
        ...
        .unwrap_or_else(|| {
            env::var("SENTRY_RELEASE")
                .unwrap_or_else(|_| format!("livicat@{}", env!("CARGO_PKG_VERSION")))
        });
    ...
    println!(
        "[Livicat] Initializing Sentry with DSN: {}...",
        &dsn[..dsn.len().min(20)]
    );
    ...
    println!(
        "[Livicat] Sentry client created, is enabled: {}",
        client.is_enabled()
    );
