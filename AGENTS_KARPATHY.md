...
### Stack
- **Language and version:** React 18 + TypeScript 5.2
- **Framework(s):** React, Vite 5.0
- **Package manager:** npm
- **Runtime / deployment target:** Desktop web app (Electron planned), OBS browser source

### Commands
- **Install:** `npm install`
- **Build:** `npm run build`
- **Test (all):** `npm run test`
- **Test (single file):** `npm run test -- path/to/test.spec.ts`
- **Lint:** `npm run lint`
...
### Layout
- **Source lives in:** `src/`
- **Tests live in:** `src/__tests__/` (to be created)
- **Do not modify:** `node_modules/`, `dist/`, `.vite/` (generated code)

### Conventions specific to this repo
- **Naming:** camelCase for variables, PascalCase for components
- **Import style:** Absolute imports from `src/`, relative for same directory
- **Error handling pattern:** Try-catch with proper logging, user-friendly error messages
- **Testing pattern and framework:** Jest + React Testing Library (to be set up)

### Forbidden
- **Direct API keys in code**: Use environment variables only
- **Hardcoded URLs**: Make them configurable
- **Secret commits**: Never commit sensitive data
- **Skipping pre-commit checks**: Always run lint and typecheck
...
- **Always verify git config before any work** - Ensure author is migorengx and email is correct
- **Never assume GitHub auth is correct** - Always verify active account before operations
- **Pre-flight checklist is mandatory** - Check git config, auth, and environment before starting
